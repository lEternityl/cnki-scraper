...
- **检索范围与口径**：高级检索走 CNKI 新版 kns8s 一框式检索（`Operator=TOPRANK`），检索范围为「学术期刊」子库，**主题等字段的匹配口径与官网检索页完全一致**（已实测同一检索词条数一致），并携带 kns8s 动态请求签名。知网官网默认检索范围是「总库」（含学位论文、会议论文、报纸等），总库条数会多于本系统，如需总库文献请使用「题录导入」功能导入官网导出的题录。系统走 sclib.cn 代理镜像，数据可能比官网略有滞后。
