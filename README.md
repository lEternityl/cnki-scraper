...
2. 选择 **EndNote / Refworks / 自定义** 任一格式，下载 txt 文件（支持 UTF-8 与 GBK 编码）；
3. 在本页签上传该文件，点击「导入」。

**导入行为：** 自动识别三种导出格式并解析（篇名、作者、刊名、发表时间、关键词、摘要、链接）；按「篇名+刊名」与本地已有记录去重；导入后自动重建全量 JSON，立即可在「文献检索」与「统计可视化」中使用。
...
| POST | `/api/records/import` | 导入官网导出的题录文件（EndNote/Refworks/自定义） |
