...
- **检索范围说明**：本系统的高级检索固定检索 CNKI「学术期刊」子库（`Resource: JOURNAL`）。知网官网默认检索范围是「总库」（含学位论文、会议论文、报纸等），因此**同一检索词下系统返回的条数会少于官网总库**，官网结果中混有的硕博/会议文献在系统里查不到，属正常差异而非数据缺失。对比时请以官网限定「学术期刊」范围的结果为基准。此外系统走 sclib.cn 代理镜像，数据可能比官网略有滞后。
