...
- **检索范围与口径**：高级检索走 CNKI 新版 kns8s 一框式检索（`Operator=TOPRANK`），检索范围为「**总库**」（学术期刊 + 学位论文 + 会议 + 报纸 + 年鉴等 10 个子库，与官网总库检索页完全一致，已实测同一检索词条数一致），并携带 kns8s 动态请求签名。采集入库的记录自动识别文献类型（期刊/学位论文/报纸/会议等），在文献检索页「数据库」列区分显示。系统走 sclib.cn 代理镜像，数据可能比官网略有滞后。
